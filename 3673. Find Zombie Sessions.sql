...
with a as (
select session_id, sum(case when event_type = 'purchase' then 1 else 0 end) as pc from app_events
group by session_id
having pc =0
)


select ap.session_id, ap.user_id, 
TIMESTAMPDIFF(MINUTE, min(ap.event_timestamp), max(ap.event_timestamp)) as session_duration_minutes,
sum(case when ap.event_type = 'scroll' then 1 else 0 end) as scroll_count


from app_events as ap
join a on 
a.session_id = ap.session_id
group by ap.session_id 
having session_duration_minutes >=30 and scroll_count>=5 
and (sum(case when ap.event_type = 'click' then 1 else 0 end)/scroll_count) <0.2
order by scroll_count desc, ap.session_id 
