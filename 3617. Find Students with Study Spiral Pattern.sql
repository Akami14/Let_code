...
with filt as (select *,
lead(session_date, 1, session_date) over (partition by student_id order by session_date) as next_date, 
lead(subject, 1, (select subject from study_sessions order by session_date limit 1)) over (partition by student_id order by session_date) as next_subject
from study_sessions),

data_filt as (
select *, 
sum(CASE WHEN DATEDIFF(next_date, session_date )<=2 THEN 1 ELSE 0 END) as cyc2 from filt 
-- filter data by no gaps longer than 2
group by student_id, subject
having cyc2 >=2
), 

subject_seq_filt as (
select student_id, subject, next_subject, sum(1) as subj_pairs from filt
group by student_id, subject, next_subject 
having subj_pairs >=2
), 


cycle_of_reapet as (
select student_id,  session_date, next_date, 
SUM(CASE WHEN DATEDIFF(next_date, session_date )<=2 THEN 1 ELSE 0 END) AS cycle_length
from data_filt
group by student_id
-- having cycle_length>=3
),
cycle_of_reapet2 as ( 
select c.student_id,  c.session_date, c.next_date, c.cycle_length, s.subj_pairs from cycle_of_reapet c
join subject_seq_filt s
on c.student_id = s.student_id
group by c.student_id)



select  a.student_id, c.student_name, c.major, a.cycle_length,  sum(b.hours_studied) as total_study_hours from cycle_of_reapet2 a
join filt b
