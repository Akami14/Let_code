...
plan_name, event_date, monthly_amount, event_type,
rank() over (partition by user_id order by event_date desc, event_type ) as rn
from subscription_events)

select t1.user_id, 
t1.plan_name  as current_plan, 
t1.monthly_amount  as current_monthly_amount, 
t2.max_historical_amount as max_historical_amount, 
t2.days_as_subscriber as days_as_subscriber
from t1 
join 
(select user_id, TIMESTAMPDIFF(day, min(event_date), max(event_date)) as days_as_subscriber,
max(monthly_amount) as max_historical_amount from subscription_events group by user_id) as t2
on t1.user_id = t2.user_id

where t1.rn=1 and t2.days_as_subscriber > 60 and t1.plan_name is not null 
and ( t1.monthly_amount/ t2.max_historical_amount ) <= 0.5
and t1.event_type != 'cancel'
order by t2.days_as_subscriber desc 
