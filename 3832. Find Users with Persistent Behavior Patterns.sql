...
*/
with a as (select *,
rank() over (partition by user_id, action order by action_date) as r1
from activity)

#select * from a
select user_id, action, max(r1) as streak_length,
min(action_date) as start_date , DATE_ADD(min(action_date), INTERVAL max(r1)-1 DAY) as end_date
from a
--where  (select max(r1) from a) >=5
