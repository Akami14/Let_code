...
d as (
select c.user_id, c.course_id, c.course_name as first_course,
lead(c.course_name) over (partition by c.user_id order by completion_date) as second_course
from course_completions as c
join b on b.user_id = c.user_id 
)

select  first_course, second_course,  count(1) as  transition_count  from d 
