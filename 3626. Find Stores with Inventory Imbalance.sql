...
With A as (
select price, store_id, product_name, quantity as pq, 
ROW_NUMBER() over (partition by store_id order by price  asc) as pr1
from inventory 
),

B as (select price, store_id, product_name, quantity as cq,
ROW_NUMBER() over (partition by store_id order by price desc) as cr1 
from inventory 
),
D as ( select store_id, count(1) as tot from inventory group by store_id
)

select C.store_id, C.store_name, C.location,
B.product_name as most_exp_product, A.product_name as cheapest_product,

round(A.pq/B.cq,2) as imbalance_ratio
from A
join B
...
where B.cr1=1 and A.pr1=1 and D.tot>=3 
having imbalance_ratio > 1
order by imbalance_ratio desc, C.store_name asc
#group by store_id
