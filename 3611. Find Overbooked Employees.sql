...
*/
with t as (
select employee_id, sum(duration_hours), WEEK(meeting_date, 1)
from meetings
group by employee_id, WEEK(meeting_date, 1 ) # week(meeting_date) DATE_FORMAT
having sum(duration_hours) >= 20 
)


select t.employee_id, e.employee_name, e.department, count(1) as meeting_heavy_weeks   from t
