...
round(sum(
    case 
    when order_rating is not null then 1 else 0 END)*100/count(1),0) as 'tr'
...
select r1.customer_id as customer_id, count(1) AS total_orders,  
round(sum(CASE WHEN (DATE_FORMAT(r1.order_timestamp, '%H:%i') between '11:00' and '14:00')
or DATE_FORMAT(r1.order_timestamp, '%H:%i')  between '18:00' and '21:00'
THEN 1 ELSE 0 END)*100 / count(1),0) as peak_hour_percentage,
round(AVG(r1.order_rating),2) AS average_rating


from restaurant_orders as r1
join t on t.customer_id = r1.customer_id
where tr>=50
group by customer_id      
having average_rating >= 4.00 and total_orders >= 3 
