...
With t3 as
(Select t1.driver_id, avg(t1.distance_km/t1.fuel_consumed) as first_half_avg,
avg(t2.distance_km/t2.fuel_consumed) as second_half_avg
From trips t1
Join trips t2 on t1.driver_id = t2.driver_id and (month(t1.trip_date) between 1 and 6) and (month(t2.trip_date) between 7 and 12 )
Group by t1.driver_id
)

Select t3.driver_id, driver_name, round(first_half_avg,2) as first_half_avg, round(second_half_avg,2) as second_half_avg, round((second_half_avg - first_half_avg),2) as efficiency_improvement
From t3
left Join drivers d on d.driver_id = t3.driver_id
