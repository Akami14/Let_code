...
with A as (
select book_id, sum(case when session_rating >=4 then 1 else 0 end) as p,
sum(case when session_rating <=2 then 1 else 0 end) as n, count(1) as tot,
max(session_rating)- min(session_rating) as rating_spread

from reading_sessions
group by book_id 
having p > 0 and n > 0 and tot>=5)


select A.book_id, B.title, B.author, B.genre, B.pages, A.rating_spread,
round((A.p+A.n)/A.tot,2) as polarization_score 
from A
join books B
