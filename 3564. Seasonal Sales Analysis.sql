...
select 
case 
when month(sale_date + INTERVAL 1 MONTH) between 1 and 3 then 'Winter'
when month(sale_date + INTERVAL 1 MONTH) between 4 and 6 then 'Spring'
when month(sale_date + INTERVAL 1 MONTH) between 7 and 9 then 'Summer'
when month(sale_date + INTERVAL 1 MONTH) between 10 and 12 then 'Fall'
end AS season, 
P.category,
SUM(S.quantity) AS total_quantity,
SUM(S.quantity * S.price) AS total_revenue
#RANK() OVER (PARTITION BY season  ORDER BY total_quantity) AS R1
from sales S
JOIN products P ON P.product_id = S.product_id
GROUP BY season, P.category ),
T1 AS ( SELECT * , RANK() OVER (PARTITION BY season ORDER BY total_quantity DESC, total_revenue  DESC) AS R1 FROM T
)


SELECT season, category, total_quantity, total_revenue  FROM T1
WHERE R1=1
order by season
