...
with t as ( 
select *,
rank() over (partition by employee_id  order by review_date desc) as r1,
lag(rating,1 ) over (partition by employee_id  order by review_date) as r2,
lag(rating,2 ) over (partition by employee_id  order by review_date) as r3
from performance_reviews )

select t.employee_id, e.name as name, (rating - r3) as improvement_score from t
join employees e on e.employee_id = t.employee_id
where r1=1 and r3 is not null and r2 is not null 
and (rating - r2)>=1 and (rating -r3) >=1 and (r2-r3)>=1
